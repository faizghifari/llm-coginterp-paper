...
- **Ragged observations.** Real rows have a variable number of observed benchmarks, so we store observations as a ragged list of column-and-value pairs per row rather than in the paper's fixed-$k$ rectangular layout. The estimator consumes observed cells only, and never a dense matrix plus a mask.
