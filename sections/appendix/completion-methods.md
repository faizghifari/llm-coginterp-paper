...
| MICE \citep{vanbuuren2011} | Multiple imputation by chained equations; produces $m$ completed datasets, averaged before factoring. | mice | $m$ | {5, 10, 20}, predictive mean matching, 5 iterations |
...
- **Ragged observations.** Real rows have a variable number of observed benchmarks, so we store observations as a ragged list of column-and-value pairs per row rather than in the paper's fixed-$k$ rectangular layout. The estimator consumes observed cells only — never a dense matrix plus a mask.
...
4. Symmetrise, then project to the nearest valid correlation matrix (the nearest positive-definite projection of \citet{higham2002}, with unit diagonal and a final eigenvalue projection), guaranteeing positive definiteness rather than near-definiteness — so every principal submatrix $R_{SS}$ is invertible.
