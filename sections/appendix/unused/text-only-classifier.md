...
## Allow-list (text-only despite a matching pattern)
...
## Deny-list (non-text despite absent or misleading metadata)
...
Removal is transitive: benchmark → its result rows → any model with zero remaining results. The integrity pass (`\hyperref[duplicate-detection-and-integrity-checks]{Appendix~\ref*{duplicate-detection-and-integrity-checks}}`{=latex}) is re-run afterwards, and the per-model aggregate columns recomputed.
