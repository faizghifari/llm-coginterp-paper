...
The corpus is assembled from published evaluation records. Sources fall into four tiers, which also define the trust ordering used for conflict resolution (`\hyperref[duplicate-detection-and-integrity-checks]{Appendix~\ref*{duplicate-detection-and-integrity-checks}}`{=latex}).
...
Two further HELM leaderboards (Audio, Image2Struct) were collected but are removed in full by the text-only restriction (`\hyperref[text-only-classifier]{Appendix~\ref*{text-only-classifier}}`{=latex}).
...
**Tier 3 — benchmark-specific leaderboards**: e.g. BigCodeBench, CRUXEval, SWE-bench, BFCL (Gorilla, UC Berkeley), VMLU, SEA-LION, PubMedQA, AlpacaEval, Video-MME (multimodal; removed by `\hyperref[text-only-classifier]{Appendix~\ref*{text-only-classifier}}`{=latex}), plus ~35 further named sources each contributing a single benchmark's leaderboard.
...
## Deductive fills (the only non-verbatim values)
