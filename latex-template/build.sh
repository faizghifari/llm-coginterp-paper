...
def expand(path, visited):
    ...
    def repl(m):
        ...
        if not resolved.endswith(".md"):
            ...
            # Rewrite to a standard markdown image. Leaving the embed in
            # Obsidian's ![[file|caption]] form does not work: pandoc's
            # wikilinks extension only understands [[link]], so the whole thing
            # is emitted as literal "[[file|caption]]" text where the figure
            # should be. As ![caption](file) pandoc builds a real figure with
            # the caption. The embed syntax can't contain "]", so the caption
            # never needs escaping; a filename with spaces needs <...>.
            caption = target.split("|", 1)[1].strip() if "|" in target else ""
...
# --columns governs when pandoc gives a table explicit p{...} column widths
# instead of plain l-columns. l-columns size to their content and never wrap,
# so any appendix table with a long prose cell silently runs off the page --
# at --columns=200 the widest ran 390pt (5.4in) past the right margin. Keeping
# the value below the width of those source rows forces wrapped p{} columns
# throughout. The cost is that a table whose markdown separator row is uniform
# (|---|---|) gets evenly-split columns rather than ones proportioned to its
# content; write the separator dashes in proportion (|---|---------|) to steer
# that. Verified by sweeping the value and counting overfull boxes.
# Turning syntax highlighting off: fenced code blocks (used in the appendix)
# would otherwise get wrapped in pandoc's Shaded/Highlighting environment,
# which this template doesn't define -- plain verbatim typesetting is fine
# for the non-code snippets these blocks actually hold. Pandoc 3.6 renamed the
# old --no-highlight switch to --syntax-highlighting=none, so pick whichever
# one this machine's pandoc actually understands (collaborators are not all on
# the same version).
if pandoc --syntax-highlighting=none -t latex /dev/null >/dev/null 2>&1; then
...
# --quiet: the plain writer has to render math as unicode and warns on every
# equation it can't ("Could not convert TeX math ..."), which says nothing
# about the .tex we actually ship -- that one passes the math through verbatim.
WORDS="$(pandoc -f markdown+wikilinks_title_after_pipe -t plain --quiet "$STRIPPED" | wc -w)"
...
if command -v pdflatex >/dev/null 2>&1; then
  ...
  (
    ...
    # read; only then can the follow-up passes resolve \cite references
    # and settle cross-references/page numbers. Skipped if bibtex isn't
    # installed -- the loop below still runs so a .tex-only or
    ...
    # How many passes it takes to settle depends on how long the document is:
    # a fixed two was enough when this was written, but the appendix has since
    # grown past the point where the table of hyperref anchors stops moving
    # after two, and the PDF was shipping with stale page numbers. Keep going
    # until LaTeX stops asking, with a cap so a document that genuinely never
    # converges can't spin forever.
